...
pub const SDL_BUFFER_SIZE = 2400;
...
pub const APU = struct {
    ...
    audio_buffer: [SDL_BUFFER_SIZE]f32,
    ...
    pub fn new() APU {
        ...
        var apu = APU{
            ...
            .audio_buffer = [_]f32{0} ** SDL_BUFFER_SIZE,
            .nr52 = NR52{
                .channel_1 = false,
                .channel_2 = false,
                .channel_3 = false,
    ...
    pub fn step(self: *APU, clock: cpu.Clock) void {
        ...
        if (self.audio_buffer_downsample_count >= CPU_SPEED_HZ) {
            ...
            if (self.audio_buffer_count >= SDL_BUFFER_SIZE) {
                ...
                while (SDL.SDL_GetQueuedAudioSize(self.sdl_audio_device) > SDL_BUFFER_SIZE * 2) {
                    SDL.SDL_Delay(1);
                }
                ...
                const res = SDL.SDL_QueueAudio(self.sdl_audio_device, &self.audio_buffer, SDL_BUFFER_SIZE * 2);
                if (res < 0) {
                    sdlPanic();
                }
                // log.debug("res = {}", .{res});
    ...
    pub fn reset_registers(self: *APU) void {
        ...
        self.audio_buffer = std.mem.zeroes([SDL_BUFFER_SIZE]f32);
        ...
        switch (addr) {
            ...
            0xFF1E => {
                ...
                if (self.channel_3.nr34.trigger and self.channel_3.dac_enabled) {
                    ...
                    // ?
                    self.channel_3.current_sample = 1;
